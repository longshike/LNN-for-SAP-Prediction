...
for i, column in enumerate(order):
    if column in df.columns:
        ...
        lr.fit(X_train, y_train)  # fit,拟合 就相当于是梯度下降

        # 模型预测
        probs = lr.predict_proba(X_test)  # 返回分类的概率
        preds = probs[:, 1]
...
plt.plot(range(1, len(auc_values) + 1), auc_values, marker='o', linestyle='-', color='b')
plt.title('LR:Relationship between Number of Features and AUC')
plt.xlabel('Number of Features')
plt.ylabel('AUC Value')
plt.grid(True)
# 保存图像，设置分辨率为300DPI
plt.savefig('LR_features.png', dpi=300, bbox_inches='tight')  # bbox_inches='tight'用于确保图像的边界紧凑
plt.show()