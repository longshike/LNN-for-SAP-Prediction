...
for i, column in enumerate(order):
    if column in df.columns:
        ...
        dct.fit(X_train, y_train)  # 拟合

        # 模型预测
        probs = dct.predict_proba(X_test)  # 返回分类的概率
        preds = probs[:, 1]
...
plt.plot(range(1, len(auc_values) + 1), auc_values, marker='s', linestyle='-', color='g')
plt.title('DCT:Relationship between Number of Features and AUC')
plt.xlabel('Number of Features')
plt.ylabel('AUC Value')
plt.grid(True)

# 保存图像，设置分辨率为300DPI
plt.savefig('DCT_features.png', dpi=300, bbox_inches='tight')  # bbox_inches='tight'用于确保图像的边界紧凑
plt.show()