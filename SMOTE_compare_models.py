...
def plot_roc_curve(fprs, tprs, aucs, labels, title):
    ...
    # 保存图像，设置分辨率为300DPI
    plt.savefig('LNN_SMOTE_auc_compare.png', dpi=300, bbox_inches='tight')  # bbox_inches='tight'用于确保图像的边界紧凑
    plt.show()
...
#for model_name, y_pred, y_test, probs_model in [('LR_with SMOTE', y_pred_lr, y_test_lr, preds_lr),
#                                               ('LR_with no SMOTE', y_pred_lr_case3, y_test_lr_case3, preds_lr_case3),
#                                              ]:
#    fpr, tpr, _ = roc_curve(y_test, probs_model)
#    auc = metrics.auc(fpr, tpr)
#    fprs.append(fpr)
...
#for model_name, y_pred, y_test, probs_model in [('DCT_with SMOTE', y_pred_dct, y_test_dct, preds_dct),
#                                               ('DCT_with no SMOTE', y_pred_dct_case3, y_test_dct_case3, preds_dct_case3),
#                                              ]:
#    fpr, tpr, _ = roc_curve(y_test, probs_model)
#    auc = metrics.auc(fpr, tpr)
#    fprs.append(fpr)
...
#for model_name, y_pred, y_test, probs_model in [('RF_with SMOTE', y_pred_rfc, y_test_rfc, preds_rfc),
#                                               ('RF_with no SMOTE', y_pred_rfc_case3, y_test_rfc_case3, preds_rfc_case3),
#                                              ]:
#    fpr, tpr, _ = roc_curve(y_test, probs_model)
#    auc = metrics.auc(fpr, tpr)
#    fprs.append(fpr)
...
#for model_name, y_pred, y_test, probs_model in [('XGBoost_with SMOTE', y_pred_xgbc, y_test_xgbc, preds_xgbc),
#                                                ('XGBoost_with no SMOTE', y_pred_xgbc_case3, y_test_xgbc_case3, preds_xgbc_case3),
#                                              ]:
#    fpr, tpr, _ = roc_curve(y_test, probs_model)
#    auc = metrics.auc(fpr, tpr)
#    fprs.append(fpr)
